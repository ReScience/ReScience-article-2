\documentclass[fleqn,10pt,lineno]{peerj}
\usepackage{calc}
\usepackage{enumitem}
\usepackage{amsmath}
...
\title{Sustainable computational science:\\the ReScience initiative}
...
\affil[$^{\star}$]{ReScience co-founder}
\affil[$^{\dagger}$]{ReScience editor}
\affil[$^{\ddagger}$]{ReScience reviewer}
\affil[$^{\S}$]{ReScience author}
\corrauthor[1]{Nicolas P. Rougier}{Nicolas.Rougier@inria.fr}
...
\begin{abstract}
  ...
  the article.  ReScience is a peer-reviewed journal that targets computational
  research and encourages the explicit replication of already published
  research, promoting new and open-source implementations in order to ensure
  that the original research can be replicated from its description. To achieve
  this goal, the whole publishing chain is radically different from other
  traditional scientific journals. ReScience resides on GitHub where each new
...
When Collberg and colleagues \citep{collberg:2016} decided to measure the extent of the problem
