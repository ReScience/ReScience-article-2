...
only 102 (i.e. less than 20\%). These low numbers reflect only the authors success at running the code. They did not check for correctness of the code (does the code actually implement what is advertised in the paper), nor the replicability of the results (does the run lead to the same results as in the paper). One example of this problem can be found in \citet{topalidou:2015a} where they tried to replicate a model from the computational neuroscience literature. Source code was not included in the supplementary section of the paper and no link to a repository was provided in the main text. When they obtained the code from the corresponding author, they found that it could not be compiled and would be difficult to repurpose.\\
