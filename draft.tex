...
\twocolumn[
  ...
  running, testing, validating, sharing and replicating results, however computational
  science lags behind.
  In the best case, authors may provide their source code as a compressed archive and they may feel confident their research
  is replicable. But this is not exactly true. Buckheit \& Donoho 1995
  proposed more than 2 decades ago that, an article about computational results is
  ...
  tools, frameworks, libraries, and data. This leads to a large number of
  combinations thereof (taking into account the version for each component) that the
  chances of having the exact same configuration as one of your colleagues are
  practically zero. This has consequences for computational
  approaches in order to make sure that computational research can be faithfully replicated.
  %
  ...
  radically different from other traditional scientific journals. ReScience
  resides on GitHub where each new implementation of a computational study is
  made available together with comments, explanations, and software tests. Each
  submission takes the form of a ``pull request'' that is publicly reviewed and
  tested in order to guarantee re-usability. ReScience is
  open and collaborative by design: everything can be modified.\\
  % \begin{small}
  % \vspace{10mm}
